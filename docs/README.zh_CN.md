...
移植的[验证状态](development/gameboy-validation.zh_CN.md)、[来源和许可证记录](development/gameboy-sources.zh_CN.md)及[清洁版 8 MiB 安装与 GitHub 提交说明](development/release/gameboy-firmware.zh_CN.md)另行维护。
...
下表描述的是上游硬件演示基线，不是当前检出目录中的 Game Boy 应用，也不是芯片数据手册中所有可能的能力。实际 Game Boy 行为和分区以根 README 及上方清洁镜像说明为准。
...
partitions.csv           当前检出目录的游戏应用、ROM、存档、NVS 和 PHY 布局
