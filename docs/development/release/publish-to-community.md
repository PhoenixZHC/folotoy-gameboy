...
This page is for the separate AI Passport community publisher workflow. The Game Boy fork's GitHub Release instead uses the [clean 8 MiB image](gameboy-firmware.md); do not substitute its destructive first-install image for an existing-device update.
