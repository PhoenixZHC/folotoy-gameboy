...
For this Game Boy fork, the GitHub Release asset is the [clean 8 MiB image](gameboy-firmware.md), not the smaller legacy merged image named below. Testing that asset overwrites the full Flash, including games, saves and pairing keys; use a blank or deliberately reset test device.
