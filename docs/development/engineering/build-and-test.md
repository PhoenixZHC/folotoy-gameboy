...
> Prefer `./tools/validate.sh --firmware` for firmware builds. This Game Boy checkout produces a clean 8 MiB first-install image at `build/FoloToy-GameBoy-8MB-clean.bin`; flashing it from `0x0` erases games, saves, settings and pairing keys. Use the app-only image at `0x10000` for a compatible existing installation. See the [Game Boy image guide](../release/gameboy-firmware.md). Treat `idf.py build` and `idf.py flash` as incremental development commands, not the default delivery path.
...
Do not upload the app-only `build/FoloToy-AI-Passport.bin` as a first-install image. For this Game Boy checkout, publish the verified clean 8 MiB image and checksum as GitHub Release assets; the smaller merged file is not a complete Flash image. Community publishing has separate requirements and should not be inferred from the GitHub workflow.
