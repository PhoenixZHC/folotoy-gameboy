...
3. **验证完整固件**：脚本按 `flash_args` 生成的实际偏移逐字节确认 bootloader、partition-table 和 app，确认使用 8 MB Flash，并检查当前配置的分区布局，随后生成 `build/FoloToy-GameBoy-8MB-clean.bin` 和 SHA-256 校验文件。当前游戏固件的引导程序、分区表、应用和初始存档分别从 `0x0`、`0x8000`、`0x10000` 和 `0x710000` 开始。详情及数据影响见[镜像说明](../release/gameboy-firmware.zh_CN.md)。
4. **上传 artifact**：每次成功构建都上传清洁版 8 MiB 镜像和 `.sha256`。普通分支只有从该分支手动运行 `workflow_dispatch` 才会构建；普通 push 不触发。
...
- `FoloToy-GameBoy-8MB-clean.bin`：恰好 8 MiB 的清洁版首次安装镜像，无 ROM、设置或手柄绑定。
- `FoloToy-GameBoy-8MB-clean.bin.sha256`：对应的 SHA-256 校验文件。
...
步骤：连接设备（USB JTAG/serial debug unit）→ 选择本 Release 的清洁镜像 `FoloToy-GameBoy-8MB-clean.bin` → 选择波特率（如 460800）→ 开始写入。目标是 8MB Flash 板卡，无需其它参数。此操作会清空游戏、存档、设置和手柄绑定；已有安装只应刷应用分区。
...
- **方法（How to build / 方法）**：如何生成并校验合并固件（`./tools/validate.sh --firmware` 或
  `idf.py build`），以及要烧录的产物文件（从 `0x0` 烧录的 `FoloToy-GameBoy-8MB-clean.bin`，且会清空已有游戏、存档及手柄绑定）。
