...
class FlashArgsTest(unittest.TestCase):
    ...
    def test_readback_allows_changed_nvs_but_rejects_changed_app(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            build_dir = Path(directory)
            (build_dir / "bootloader").mkdir()
            (build_dir / "partition_table").mkdir()
            files = {
                "bootloader/bootloader.bin": (0, b"BOOT"),
                "partition_table/partition-table.bin": (0x8000, b"PART"),
                "FoloToy-AI-Passport.bin": (0x10000, b"GAME"),
            }
            readback = bytearray(b"\xff" * 0x10004)
            offsets = {}
            for name, (offset, data) in files.items():
                (build_dir / name).write_bytes(data)
                readback[offset : offset + len(data)] = data
                offsets[name] = offset
            readback[0x9000] = 0x42
            VERIFY.verify_readback_images(bytes(readback), build_dir, offsets)
            readback[0x10001] ^= 1
            with self.assertRaisesRegex(ValueError, "App|FoloToy"):
                VERIFY.verify_readback_images(bytes(readback), build_dir, offsets)
