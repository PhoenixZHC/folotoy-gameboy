...
static void audio_cleanup(void) {
    ...
    s_bits = 0;
    s_ch = 0;
    s_playback_only = false;
}

static esp_err_t i2s_channels_init(bool playback_only) {
    i2s_chan_config_t chan = {
        .id = BSP_I2S_PORT,
        .role = I2S_ROLE_MASTER,
        .dma_desc_num = playback_only ? 4 : 6,
        .dma_frame_num = playback_only ? 128 : 240,
    ...
    if (e == ESP_OK && s_rx) e = i2s_channel_enable(s_rx);
...
static esp_err_t audio_init_mode(bool playback_only) {
    ...
    if ((e = i2s_channels_init(playback_only)) != ESP_OK) goto fail;
...
esp_err_t bsp_audio_read(void *pcm, size_t bytes) {
    if (!s_dev || !s_opened || s_sleeping || s_playback_only) return ESP_ERR_INVALID_STATE;
