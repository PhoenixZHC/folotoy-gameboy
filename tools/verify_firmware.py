...
def verify_readback_images(
    readback: bytes, build_dir: Path, image_offsets: dict[str, int]
) -> None:
    """Compare immutable flashed images; NVS/PHY may change after boot."""
    for relative_name in REQUIRED_IMAGES:
        offset = image_offsets[relative_name]
        image = (build_dir / relative_name).read_bytes()
        if readback[offset : offset + len(image)] != image:
            raise ValueError(f"{relative_name} differs in device readback at 0x{offset:x}")
...
def main() -> int:
    ...
    readback_path = Path(sys.argv[2]).resolve() if len(sys.argv) > 2 else None
    ...
    if readback_path:
        try:
            verify_readback_images(readback_path.read_bytes(), build_dir, image_offsets)
        except (OSError, ValueError) as error:
            print(f"ERROR: {error}", file=sys.stderr)
            return 1
        print("Device readback images: PASS")
    return 0
